...
def txtformat(text):
    ...
    for line in text:
        if line == '\n' or line[0] == ';':
            ...
            ret.append("")
            for obj in linelis:
                if linelis.index(obj) > 0 and linelis.index(obj) < 4:
                    ret[len(ret) - 1] += obj
                else:
...
class Otmg(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.indexright = 0
        self.indexleft = 7
        self.anim_delay = 9999
        self.anim_direction = "r"
        self.animcall = Clock.schedule_once(self.anim_callback, self.anim_delay)
        ...
        self.row2.gear_left = Image(source='Clockwise/Gear0.png', size_hint=(1, 1))
        ...
        self.row2.gear_right = Image(source='Clockwise/Gear0.png', size_hint=(1, 1))
    ...
    def do_restart(self, val):
        print(self.lasttape)
        print(self.changetape)
    ...
    def set_location(self, touch, value):
        if self.lastrightknob != self.row2.knob_right.value:
            ...
            if self.state == "Stop":
                self.movedloc = True
                prev = self.otm.index
                self.otm.index = int(round(((len(self.otm.tape) - 1) / 100) * self.row2.knob_right.value))
                self.row1.tape.text=self.otm.croptape(25)
    ...
    def do_step(self, value):
        if self.changetape == True:
            self.changetape = False
            self.lasttape = self.otm.tape
    ...
    def do_startstop(self, value):
        if self.state == "Run":
            ...
            self.anim_delay_set(self.row3.slider.speed())

    def runstep_callback(self, dt):
        if self.state == "Run":
            self.do_step(None)
            Clock.schedule_once(self.runstep_callback, self.row3.slider.speed() * 5)
            if self.anim_delay == 9999:
                self.anim_delay_set(self.row3.slider.speed())
            else:
                self.anim_delay = self.row3.slider.speed()

    def anim_callback(self, val):
        self.animcall = Clock.schedule_once(self.anim_callback, self.anim_delay)
        self.indexright += 2
        if self.indexright > 7:
            self.indexright = 0
        self.indexleft -= 2
        if self.indexleft < 0:
            self.indexleft = 7
            
        if self.anim_direction == "r":
            self.row2.gear_left.source = "Clockwise/Gear" + str(self.indexright) + ".png"
            self.row2.gear_right.source = "Clockwise/Gear" + str(self.indexright) + ".png"
        else:
            self.row2.gear_left.source = "Clockwise/Gear" + str(self.indexleft) + ".png"
            self.row2.gear_right.source = "Clockwise/Gear" + str(self.indexleft) + ".png"

    def anim_delay_set(self, anim):
        Clock.unschedule(self.animcall)
        self.animcall = Clock.schedule_once(self.anim_callback, anim)
