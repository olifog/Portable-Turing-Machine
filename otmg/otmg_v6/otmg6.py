...
class Otmg(BoxLayout):
    ...
    def do_step(self, value):
        if self.geardir == "CW":
            new_num = int(self.row2.gear_left.source[14]) + 1
            if new_num < 8:
                self.row2.gear_left.source = "Clockwise/Gear" + str(new_num) + ".png"
                self.row2.gear_right.source = "Clockwise/Gear" + str(new_num) + ".png"
            else:
                self.row2.gear_left.source = "Clockwise/Gear0.png"
                self.row2.gear_right.source = "Clockwise/Gear0.png"
        else:
            new_num = int(self.row2.gear_left.source[14]) - 1
            if new_num > 0:
                self.row2.gear_left.source = "Clockwise/Gear" + str(new_num) + ".png"
                ...
                self.row2.gear_left.source = "Clockwise/Gear7.png"
                self.row2.gear_right.source = "Clockwise/Gear7.png"
    ...
    def runstep_callback(self, dt):
        if self.state == "Run":
            self.do_step(None)
            Clock.schedule_once(self.runstep_callback, self.row3.slider.speed() * 0.05)

    #def step_callback(self, dt)
